...
of :class:`imaginary`.  Complex numbers are represented as a pair of floating
point numbers and have the same restrictions on their range.  To create a
complex number with a nonzero real part, add an integer or floating point number
to imaginary, e.g., ``3+4j``.  Some examples of imaginary literals::

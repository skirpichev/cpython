...
/*[clinic end generated code: output=0ebc9836e4d302d4 input=bec1271a33d47e67]*/
{
    ...
    char *buf = PyOS_double_to_string(x, 'a', -1, 0, NULL);
...
/*[clinic end generated code: output=46c0274d22b78e82 input=0407bebd354bca89]*/
{
    ...
    const char* s = PyUnicode_AsUTF8AndSize(string, &length);
    if (s == NULL)
        return NULL;
    char *s_end = (char*)s + length, *se;

    while (Py_ISSPACE(*s))
        s++;

    errno = 0;
    double x = _Py_dg_strtod_hex(s, &se);

    if (errno == EDOM && s == se) {
        x = _Py_parse_inf_or_nan(s, &se);
        if (se != s) {
            errno = 0;
        }
    }

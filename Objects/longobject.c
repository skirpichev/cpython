...
zz_from_int(PyObject *obj, zz_t *r)
{
    ...
    assert(long_export.digits);

    zz_err ret = zz_import((size_t)long_export.ndigits,
                               long_export.digits, *int_layout, r);
...
static PyObject *
zz_to_int(const zz_t *z)
{
    const zz_layout *int_layout = (zz_layout *)PyLong_GetNativeLayout();
    size_t size = (zz_bitlen(z) + int_layout->bits_per_digit
                   - 1)/int_layout->bits_per_digit;
    void *digits;
    PyLongWriter *writer = PyLongWriter_Create(zz_isneg(z),
                                               (Py_ssize_t)size, &digits);

    if (!writer) {
        return NULL;
    }
    (void)zz_export(z, *int_layout, size, digits);
...
long_mul(PyLongObject *a, PyLongObject *b)
{
    ...
    /* Use gradeschool math when numbers are too small. */
    if (asize <= LIBZZ_MUL_CUTOFF) {
        if (asize == 0) {
            return (PyLongObject *)PyLong_FromLong(0);
        }
    ...
    if (zz_init(&at) || zz_init(&bt) || zz_init(&r)
        || zz_from_int((PyObject *)a, &at)
        || zz_from_int((PyObject *)b, &bt)
        || zz_mul(&at, &bt, &r))
    {
        zz_clear(&at);
        zz_clear(&bt);
    ...
    PyLongObject *z = (PyLongObject *)zz_to_int(&r);
